...
async function getEupmyeondongList(codeSido, codeSigungu) {
    ...
    eupmyeondongBox.appendChild(allEupmyeondongInput);
    eupmyeondongBox.appendChild(allEupmyeondongLabel);
    ...
    for (let i in eupmyeondongInfo) {
        ...
        eupmyeondongInput.addEventListener('click', function () {
            if (this.checked === false) {
                allEupmyeondongInput.checked = false;
            }
            ////////////////////////////////////////////////////////////////////////
            const code = this.id;
            if (this.checked === true && !codeEupmyeondong.includes(code)) {
                codeEupmyeondong.push(code);
            } else if (this.checked === false && codeEupmyeondong.includes(code)) {
                codeEupmyeondong.splice(codeEupmyeondong.indexOf(code), 1);
            }
    ...
    allEupmyeondongInput.addEventListener('click', function () {
        if (this.checked === true) {
            for (let i of eupmyeondongBox.childNodes) {
                i.checked = true;
                if (i.id != 'allEupmyeondong' && i.id != '') {
                    codeEupmyeondong.push(i.id);
                }
            }
        } else {
            for (let i of eupmyeondongBox.childNodes) {
        ...
        console.log('[allEupmyeondongInput-addEventListener-click]');
        console.log(codeEupmyeondong);
    });
}
